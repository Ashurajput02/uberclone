...
module.exports.authCaptain = async (req, res, next) => {
    console.log("hello i m in auth captain middleware starting mein");
    //checking for token
    //token can be in cookie or in header
    const token = req.cookies.token || req.headers.authorization.split(' ')[1];
    //ab iss token ko verify karna hoga
    ...
    try {
        ...
        //find the user by id
        console.log("Decoded token:", decoded);
        const captain = await captainModel.findById(decoded._id); // exclude password from the user object
        if (!captain) {
            return res.status(404).json({ message: "Captain not found" });
        }
        //attach user to request object
        console.log("Authenticated captain:", captain);
        req.captain = captain;//taki next middleware ko req mein captain ka poora data mile 
